// SafeKid TG Server v3 — zero dependencies + HTTPS
...
const PORT_HTTPS = parseInt(process.env.SK_PORT)  || 3000;
const PORT_HTTP  = parseInt(process.env.SK_PORT_HTTP) || 3001; // redirect ke HTTPS
const PUB   = process.env.SK_PUBLIC  || path.join(__dirname, 'public');
const BOT   = process.env.BOT_TOKEN  || '';
const CID   = process.env.CHAT_ID    || '';
const PURL  = process.env.PUBLIC_URL || '';
const DIR   = process.env.SK_DIR     || __dirname;

// ── Generate self-signed cert jika belum ada ──
const CERT_KEY = path.join(DIR, 'ssl.key');
const CERT_CRT = path.join(DIR, 'ssl.crt');

function genCert(ip) {
  try {
    const san = ip ? `IP:${ip},IP:127.0.0.1` : 'IP:127.0.0.1';
    spawnSync('openssl', [
      ...
      '-days',   '365',
      '-nodes',
      '-subj',   '/CN=safekid.local',
      '-addext', `subjectAltName=${san}`
    ], { stdio:'ignore' });
    console.log('[SSL] Certificate generated for IP: '+(ip||'localhost'));
    return true;
  } catch(e) {
    console.log('[SSL] openssl not available, HTTPS disabled');
    return false;
  }
}
...
if (!fs.existsSync(CERT_KEY) || !fs.existsSync(CERT_CRT)) genCert(LOCAL_IP);
if (fs.existsSync(CERT_KEY) && fs.existsSync(CERT_CRT)) {
  try {
    sslOpts = { key: fs.readFileSync(CERT_KEY), cert: fs.readFileSync(CERT_CRT) };
    console.log('[SSL] Certificate loaded');
  } catch(e) { console.log('[SSL] Failed to load cert:', e.message); }
}
...
// ── Start servers ──
if (sslOpts) {
  // HTTPS server utama — kamera & lokasi bisa jalan
  const httpsServer = https.createServer(sslOpts, reqHandler);
  ...
  const httpRedir = http.createServer((req, res) => {
    const host = (req.headers.host || '').split(':')[0] || LOCAL_IP || 'localhost';
    res.writeHead(301, { Location: 'https://'+host+':'+PORT_HTTPS+req.url });
    res.end();
  });
  httpRedir.on('upgrade', wsUpgrade); // support WS via HTTP juga (fallback)
  httpRedir.on('error', e => console.error('[HTTP]', e.message));

  httpsServer.listen(PORT_HTTPS, '0.0.0.0', () => {
    console.log('✅ SafeKid HTTPS ready → https://'+LOCAL_IP+':'+PORT_HTTPS);
    if (PURL) tgMsg('🛡️ <b>SafeKid Aktif!</b>\n\n🎂 Link Ulang Tahun:\n<code>'+PURL+'/birthday.html?r=BDAY&n=Nama</code>\n\n/help untuk perintah bot');
    setTimeout(poll, 2000);
  });

  httpRedir.listen(PORT_HTTP, '0.0.0.0', () => {
    console.log('✅ SafeKid HTTP redirect on port '+PORT_HTTP);
  });

} else {
  // Fallback HTTP jika openssl tidak ada
  const server = http.createServer(reqHandler);
  server.on('upgrade', wsUpgrade);
  server.listen(PORT_HTTPS, '0.0.0.0', () => {
    console.log('⚠️  SafeKid HTTP only (no SSL) on port '+PORT_HTTPS);
    if (PURL) tgMsg('🛡️ <b>SafeKid Aktif!</b>\n\n🎂 Link Ulang Tahun:\n<code>'+PURL+'/birthday.html?r=BDAY&n=Nama</code>\n\n/help untuk perintah bot');
    setTimeout(poll, 2000);
  });
}
